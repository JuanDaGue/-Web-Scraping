...
(async () => {
    ...
    try {
        let jobs = [];
        msg(window.location.origin + '/wday/cxs' + window.location.pathname.replace(/\/recruiting\//g, '/') + '/jobs')
        let resp = await fetch(window.location.origin + '/wday/cxs' + window.location.pathname.replace(/\/recruiting\//g, '/') + '/jobs', {
            "headers": {
                "accept": "application/json",
                "accept-language": "en-US",
                "content-type": "application/json",
        ...
        for (let i in htmlJobs) {
            ...
            let job = {};
            job.title = elem.title;
            job.url = window.location.origin + window.location.pathname + elem.externalPath;
            msg('Title: ' + job.title + '\nURL: ' + job.url);
            job.temp = 96;
            ///////////////////////////////
            let respDec = await fetch(window.location.origin + '/wday/cxs' + window.location.pathname.replace(/\/recruiting\//g, '/') + elem.externalPath, {
                "headers": {
                    "accept": "application/json",
                    "accept-language": "en-US",
                    "content-type": "application/x-www-form-urlencoded",
